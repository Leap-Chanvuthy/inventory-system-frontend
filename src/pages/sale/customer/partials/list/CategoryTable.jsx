...
const CategoryTable = ({ filters }) => {
  ...
  const handleDelete = async () => {
    ...
    setStatus("loading");
    try {
      const response = await axios.delete(`${BASE_URL}/customer-category/delete/${selectedId}`);
                ...
                >
                  ...
                  <Table.Cell className="whitespace-nowrap">
                    {new Date(category.created_at).toLocaleString('en-US', {
                        ...
                        hour: '2-digit',
                        minute: '2-digit',
                    })}, {formatDistanceToNow(new Date(category.created_at))} ago
                  </Table.Cell>
                  <Table.Cell className="whitespace-nowrap">
                    {formatDistanceToNow(new Date(category.updated_at))} ago
                  </Table.Cell>
                  <Table.Cell className="flex items-center cursor-pointer gap-3 whitespace-nowrap font-medium text-gray-900 dark:text-white">
                    <div onClick={() =>{setSelectedId(category.id)}}>
                        <Update customer_category_id={selectedId || null} />
                    </div>
                    <MdDelete
